...
### Detalles de implementación LSI
...
**Nota importante sobre sklearn:** sklearn produce matrices (documentos × términos), que es la transpuesta de la notación matemática clásica (términos × documentos). Esto no afecta el resultado pero hay que ser consciente al implementar la proyección de queries.

**Selección de k (dimensiones latentes):**
- k muy bajo (10-30): Pierde información, agrupa demasiado.
- k muy alto (500+): No reduce el ruido, pierde el beneficio de LSI.
- Para corpus médico de tamaño medio: empezar con k=100, ajustar con evaluación.

**Punto crítico:** Si la query contiene palabras que NO están en el vocabulario del vectorizer, estas se ignoran silenciosamente. Por eso se usa el Trie de corrección ortográfica ANTES de la vectorización.

**Persistencia:** Guardar vectorizer, SVD y doc_vectors con `joblib.dump` y `np.save` para no recalcular cada vez.
...
## Creación de módulos y plugins

**Decisión inicial:** Un **módulo** (`modules/`) es parte obligatoria del pipeline y siempre se ejecuta. Un **plugin** (`plugins/`) es opcional, se engancha en un hook, y si no se registra el pipeline lo ignora.
...
├── models.py            # Modelos de datos específicos (opcional)
└── [archivos_extra].py  # Lógica específica
```

**Checklist antes de dar por terminado un módulo:**
- La interfaz ABC está definida en `core/interfaces.py`.
...
- Si es plugin: `hook_name` es uno de los válidos y `execute` recibe y devuelve `PipelineContext`.

## Testing

Cada módulo es testeable de forma aislada gracias a las interfaces ABC.
...
├── integration/             # Pipeline completo y plugins
└── evaluation/              # Queries de prueba + métricas (Corte 3)
```

**Mocks:** Crear implementaciones fake de las interfaces ABC para tests unitarios. Ejemplo: un `MockRetriever` que devuelve documentos predefinidos.

**Ejecutar:** `python -m pytest tests/ -v`

## Docker

Es un monolito modular = **un solo servicio** en Docker. El Dockerfile debe existir desde el Corte 1.

**Reglas:**
- Todo debe funcionar con `docker-compose up --build`. Sin pasos manuales adicionales.
- No dependencias en la nube. Todo local y reproducible.
- Volúmenes para datos persistentes (corpus y modelos entrenados sobreviven a rebuilds).
...
- Fuente bibliográfica del LSI: Deerwester et al., 1990 — "Indexing by latent semantic analysis", JASIS vol. 41(6).
...
Para cualquier trabajo nuevo, siempre sugerir crear una rama desde `dev`:
...
2. **Planeo Estratégico y Aprobación**
   ...
   - Verificación del usuario: Presenta tu plan y espera aprobación o modificaciones antes de proceder con la implementación.
